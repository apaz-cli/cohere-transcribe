/* backends/iron/backend.c — IRON (AMD XDNA / Strix Halo) backend.
 *
 * Strix Halo shares CPU/NPU address space, so no HtoD/DtoH copies are needed.
 * All compute currently falls back to the CPU scalar implementation; individual
 * functions will be replaced with AIE kernel dispatch once the XRT SDK and
 * mlir-aie toolchain are available.
 *
 * backend_init / backend_destroy stub the XRT device context.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static _Thread_local int g_iron_S_batch[MAX_BATCH];
static _Thread_local int g_iron_T_enc_batch[MAX_BATCH];
...
/* Unified memory: weights stay in the mmap'd host region. */
...
/* Shared address space: plain malloc/free suffices. */
...
void backend_htod(float *dev_dst, const float *host_src, size_t floats) {
    (void)dev_dst; (void)host_src; (void)floats;
}
...
/* ============================================================ */
/* Kernel-boundary functions — CPU scalar fallback              */
/* ============================================================ */
...
void backend_embed_decode_batch(float *h, const int *tokens,
                                 ...
                                 const uint16_t *etw, const uint16_t *epw,
                                 const uint16_t *elnw, const uint16_t *elnb) {
    memcpy(g_iron_S_batch, S_dev, (size_t)B * sizeof(int));
    for (int b = 0; b < B; b++) {
...
void backend_upload_T_enc(const int *T_enc, int B) {
    memcpy(g_iron_T_enc_batch, T_enc, (size_t)B * sizeof(int));
}
...
void backend_sdp_attn_batch_decode_ca(float *out, const float *Q,
    ...
    for (int b = 0; b < B; b++) {
        backend_sdp_attn(out + (size_t)b * DEC_D,
                         ...
                         K   + (size_t)b * T_k_stride * DEC_D,
                         V   + (size_t)b * T_k_stride * DEC_D,
                         NULL, 1, g_iron_T_enc_batch[b], work);
    }
}
